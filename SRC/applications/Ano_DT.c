...
{	
	//串口1发送配置/////////////////////////////////////////////////////////////
//	//PWM
	dt.txSet_u2[CSID_X20].fre_ms = 10; //10ms
	//ACC-GRO
...
void AnoDTRxOneByte(u8 type,u8 data)
{
	...
	switch (rx_anl[_pbuf].rxstate)
	{
		case 0:
		{
			if(data==0xAA)
			{
				rx_anl[_pbuf].rxstate=1;
				rx_anl[_pbuf].DT_RxBuffer[0]=data;
			}
		}
		break;
		case 1:
		{
			...
			{
				rx_anl[_pbuf].rxstate=2;
				rx_anl[_pbuf].DT_RxBuffer[1]=data;
			}
			else
			{
				rx_anl[_pbuf].rxstate = 0;
			}			
		}
		break;
		case 2:
		{
			rx_anl[_pbuf].rxstate = 3;
			rx_anl[_pbuf].DT_RxBuffer[2]=data;	
		}
		break;
		case 3:
		{
			...
			{
				rx_anl[_pbuf].rxstate = 4;
				rx_anl[_pbuf].DT_RxBuffer[3]=data;
				rx_anl[_pbuf]._data_len = data;
				rx_anl[_pbuf]._data_cnt = 0;
			}
			else
			{
				rx_anl[_pbuf].rxstate = 0;
		...
		case 4:
		{
			if(rx_anl[_pbuf]._data_len>0)
			{
				rx_anl[_pbuf]._data_len--;
				rx_anl[_pbuf].DT_RxBuffer[4+rx_anl[_pbuf]._data_cnt++]=data;
				if(rx_anl[_pbuf]._data_len==0)
				{
					rx_anl[_pbuf].rxstate = 5;
				}
			}
			else
			{
		...
		case 6:
		{
			...
			rx_anl[_pbuf].rxstate = 0;
			rx_anl[_pbuf].DT_RxBuffer[4+rx_anl[_pbuf]._data_cnt]=data;
			_data_cnt = rx_anl[_pbuf]._data_cnt+5;
			//ano_dt_data_ok = 1;
			AnoDTDataAnl(type, rx_anl[_pbuf].DT_RxBuffer, _data_cnt);		
		}
		break;	
		default :
		{
